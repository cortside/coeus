...
from .post_api_v1_orders_models import post_api_v1_orders_Input, post_api_v1_orders_Output
from utils.output_helper import create_output
from typing import get_args
tracer = get_tracer("acme.shoppingcart.mcp.tool.post_api_v1_orders")
@mcp.tool(name="post_api_v1_orders", description="Create a new order (Auth permission: CreateOrder)", tags={"route:/api/v1/orders", "method:POST"})
async def post_api_v1_orders(input: post_api_v1_orders_Input) -> post_api_v1_orders_Output:
    ...
    try:
        ...
        with tracer.start_as_current_span("tool_call") as span:
            ...
            res = await call_api("POST", f"/api/v1/orders", params=query_params, json=input.body, tool="post_api_v1_orders")
            tool_invocations_total.labels(tool="post_api_v1_orders", outcome="success").inc()
            return create_output(post_api_v1_orders_Output, res)
    except ValidationError:
        tool_validation_errors_total.labels(tool="post_api_v1_orders").inc()
        return post_api_v1_orders_Output()
    except Exception:
