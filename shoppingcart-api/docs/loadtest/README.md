...
If indexes and stats are rebuilt with deploy of changed code, from my understanding I don't expect index fragmentation or average space used per page to change to be worse than if stating the table from new.  I would expect the pages to be inserted into wherever in the btree they go and then grow from there being sequential.
