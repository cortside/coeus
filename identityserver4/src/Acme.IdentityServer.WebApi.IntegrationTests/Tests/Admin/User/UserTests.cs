...
using Acme.IdentityServer.WebApi.IntegrationTests;
...
namespace Acme.IdentityServer.WebApi.IntegrationTests.Tests.User {
    ...
    public class UserTest : IClassFixture<IntegrationTestFixture> {
        ...
        public async Task ShouldCreateUserAsync() {
            ...
            // assert
            response.UserStatus.Should().Be(Models.Enumerations.UserStatus.New);
        }
        ...
        public async Task ShouldUpdateUserAsync() {
            ...
            var updateUserRespData = JsonConvert.DeserializeObject<Models.Output.UserOutputModel>(await updateUserResp.Content.ReadAsStringAsync().ConfigureAwait(false));
        ...
        private async Task<Models.Output.UserOutputModel> CreateUserAsync(Models.Input.CreateUserModel user) {
            integrationTestFixture.Client.DefaultRequestHeaders.Authorization = await GetTokenHeaderAsync();
