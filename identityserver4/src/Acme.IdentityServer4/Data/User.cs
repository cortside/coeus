...
namespace EnerBank.IdentityServer.Data {
    public class User : AuditableEntity {
        ...
        public void UpdateExternal(List<Claim> claims) {
            ...
            // remove name claim that matches username if it exists
            var username = filtered.FirstOrDefault(x => x.Type == JwtClaimTypes.Name && x.Value == this.Username);
            if (filtered.Count(x => x.Type == JwtClaimTypes.Name) > 1 && username != null) {
            ...
            var unc = this.UserClaims.FirstOrDefault(x => x.Type == JwtClaimTypes.Name && x.Value == username.Value);
            if (this.UserClaims.Count(x => x.Type == JwtClaimTypes.Name) > 1 && unc != null) {
                claimsToRemove.Add(unc);
            }
            claimsToRemove.ForEach(c => this.UserClaims.Remove(c));
