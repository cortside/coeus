...
using EnerBank.IdentityServer.WebApi.Services;
...
namespace EnerBank.IdentityServer.Services {
    public class ClientService : IClientService {
        ...
        public Client UpdateClient(int id, UpdateClientModel request) {
            ...
            if (client == null) {
                throw new BadRequestException($"Updating a client that does not exist is not allowed.");
            }

            if (request.Claims != null && request.Claims.Any(x => x.Type == "sub")) {
                throw new BadRequestException($"Updating client sub claim is not allowed");
            }

            if (string.IsNullOrEmpty(request.Email)) {
                throw new BadRequestException($"Updating a client without providing an Email is not allowed.");
            }

            if (string.IsNullOrEmpty(request.PhoneNumber)) {
                throw new BadRequestException($"Updating a client without providing a PhoneNumber is not allowed.");
            }
        ...
        public Client CreateClient(CreateClientModel request) {
            ...
            try {
                ...
                if (string.IsNullOrEmpty(request.SubjectId)) {
                    throw new BadRequestException($"Creating a new client without providing a SubjectId is not allowed.");
                }

                if (string.IsNullOrEmpty(request.Email)) {
                    throw new BadRequestException($"Creating a new client without providing an Email is not allowed.");
                }

                if (string.IsNullOrEmpty(request.PhoneNumber)) {
                    throw new BadRequestException($"Creating a new client without providing a PhoneNumber is not allowed.");
                }

                if (string.IsNullOrEmpty(request.ClientId)) {
                    throw new BadRequestException($"Creating a new client without providing a ClientId is not allowed.");
                }
                ...
                if (db.Clients.Any(x => x.ClientId == request.ClientId) || db.ClientClaims.Any(x => x.Type == "sub" && x.Value == request.SubjectId)) {
                    throw new BadRequestException($"ClientId/SubjectId {request.ClientId}/{request.SubjectId} already exists, cannot create");
                }
