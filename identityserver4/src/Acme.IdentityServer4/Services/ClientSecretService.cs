...
namespace Cortside.IdentityServer.WebApi.Services {
    public class ClientSecretService : IClientSecretService {
        ...
        private readonly IHtmlTemplateHelper htmlTemplateHelper;
        private readonly IConfigurationRoot config;
        private readonly IHashProvider hashProvider;

        public ClientSecretService(IServiceProvider serviceProvider, ILogger<ClientSecretService> logger, IHtmlTemplateHelper htmlTemplateHelper, IConfigurationRoot config, IHashProvider hashProvider) {
            this.serviceProvider = serviceProvider;
            this.logger = logger;
            this.htmlTemplateHelper = htmlTemplateHelper;
            this.config = config;
        ...
        public async Task SendClientSecretEmail(int clientId) {
            ...
            if (client == null) {
                throw new BadRequestException($"ClientId: {clientId} cannot have a secret reset because it does not exist.");
            }

            var emailClaim = db.ClientClaims.FirstOrDefault(x => x.ClientId == client.Id && x.Type == "email");
            if (emailClaim == null) {
                throw new BadRequestException($"ClientId: {clientId} cannot have it's secret reset because it doesn't have an email claim.");
            }
            ...
            if (clientSecretRequest == null) {
                throw new BadRequestException($"ClientId: {clientId} cannot have it's secret reset because it doesn't have a secret request.");
            }
            ...
            var tokens = new List<HtmlTemplateParameter>() {
            ...
            var body = htmlTemplateHelper.GetHtmlTemplate(HtmlTemplateType.ClientSecretEmail, tokens);

            EmailRequestEvent @event = new EmailRequestEvent() {
                Recipients = new string[] { emailClaim.Value },
                MessagePayload = body,
                Subject = "CortsideUSA API Integration Instructions",
                From = "NoReply@CortsideUSA.com"
            };
        ...
        public async Task<Client> ResetSecret(int clientId) {
            ...
            if (clientSecret == null) {
                throw new BadRequestException($"ClientId: {clientId} cannot have a secret reset because a current secret does not exist");
            }
        ...
        public async Task SendVerificationCode(Guid clientSecretRequestId, SendVerificationCodeModel sendVerificationCodeModel) {
            ...
            if (clientSecretRequest == null) {
                throw new BadRequestException($"Cannot send verification code for ClientSecretRequestId: {clientSecretRequestId} because the client secret request doesn't exist.");
            }

            if (clientSecretRequest.RequestExpirationDate < DateTime.Now) {
                throw new BadRequestException($"ClientId: {clientSecretRequest.ClientId} cannot send verification code because secret request is expired.");
            }
            ...
            if (sendVerificationCodeModel.TokenHash != tokenHashed) {
                throw new BadRequestException($"ClientId: {clientSecretRequest.ClientId} cannot send verification code because token hash is invalid.");
            }
            ...
            if (client == null) {
                throw new BadRequestException($"ClientId: {client.ClientId} cannot send verification code because it does not exist.");
            }
            ...
            var payload = $"Cortside USA automated msg: Your verification code is: {code}. It’ll expire in {expirationInMinutes} min. Don’t share this with anyone.";
