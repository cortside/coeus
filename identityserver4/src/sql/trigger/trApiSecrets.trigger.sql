DROP TRIGGER IF EXISTS AUTH.trApiSecrets
GO

---
-- Trigger for ApiSecrets that will handle logging of both update and delete
-- NOTE: inserted data is current value in row if not altered
---
CREATE TRIGGER trApiSecrets
	ON [AUTH].[ApiSecrets]
	FOR UPDATE, DELETE
	AS
		BEGIN
	...
	values('ApiSecrets', 'AUTH', @action, CASE WHEN LEN(HOST_NAME()) < 1 THEN ' ' ELSE HOST_NAME() END,
