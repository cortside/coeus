DROP TRIGGER IF EXISTS AUTH.trApiScopeProperties
GO

---
-- Trigger for ApiScopeProperties that will handle logging of both update and delete
-- NOTE: inserted data is current value in row if not altered
---
CREATE TRIGGER trApiScopeProperties
	ON [AUTH].[ApiScopeProperties]
	FOR UPDATE, DELETE
	AS
		BEGIN
	SET NOCOUNT ON
	...
	values('ApiScopeProperties', 'AUTH', @action, CASE WHEN LEN(HOST_NAME()) < 1 THEN ' ' ELSE HOST_NAME() END,
	...
	-- [ScopeId]
	IF UPDATE([ScopeId]) OR @action in ('INSERT', 'DELETE')      
		BEGIN       
			INSERT INTO audit.AuditLog (AuditLogTransactionId, PrimaryKey, ColumnName, OldValue, NewValue, Key1)
			SELECT
				@AuditLogTransactionId,
				convert(nvarchar(1500), IsNull('[[Id]]='+CONVERT(nvarchar(4000), IsNull(OLD.[Id], NEW.[Id]), 0), '[[Id]] Is Null')),
				'[ScopeId]',
				CONVERT(nvarchar(4000), OLD.[ScopeId], 126),
				CONVERT(nvarchar(4000), NEW.[ScopeId], 126),
				convert(nvarchar(4000), COALESCE(OLD.[Id], NEW.[Id], null))
			FROM deleted OLD 
			LEFT JOIN inserted NEW On (NEW.[Id] = OLD.[Id] or (NEW.[Id] Is Null and OLD.[Id] Is Null))
			WHERE ((NEW.[ScopeId] <> OLD.[ScopeId]) 
					Or (NEW.[ScopeId] Is Null And OLD.[ScopeId] Is Not Null)
					Or (NEW.[ScopeId] Is Not Null And OLD.[ScopeId] Is Null))
			set @inserted = @inserted + @@ROWCOUNT
		END
