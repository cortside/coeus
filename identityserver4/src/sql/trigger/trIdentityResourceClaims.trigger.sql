DROP TRIGGER IF EXISTS AUTH.trIdentityResourceClaims
GO

---
-- Trigger for IdentityResourceClaims that will handle logging of both update and delete
-- NOTE: inserted data is current value in row if not altered
---
CREATE TRIGGER trIdentityResourceClaims
	ON [AUTH].[IdentityResourceClaims]
	FOR UPDATE, DELETE
	AS
		BEGIN
	SET NOCOUNT ON
	...
	values('IdentityResourceClaims', 'AUTH', @action, CASE WHEN LEN(HOST_NAME()) < 1 THEN ' ' ELSE HOST_NAME() END,
