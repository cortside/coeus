DROP TRIGGER IF EXISTS AUTH.trApiScopes
GO

---
-- Trigger for ApiScopes that will handle logging of both update and delete
-- NOTE: inserted data is current value in row if not altered
---
CREATE TRIGGER trApiScopes
	ON [AUTH].[ApiScopes]
	FOR UPDATE, DELETE
	AS
		BEGIN
	...
	values('ApiScopes', 'AUTH', @action, CASE WHEN LEN(HOST_NAME()) < 1 THEN ' ' ELSE HOST_NAME() END,
