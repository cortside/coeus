DROP TRIGGER IF EXISTS AUTH.trClientProperties
GO

---
-- Trigger for ClientProperties that will handle logging of both update and delete
-- NOTE: inserted data is current value in row if not altered
---
CREATE TRIGGER trClientProperties
	ON [AUTH].[ClientProperties]
	FOR UPDATE, DELETE
	AS
		BEGIN
	SET NOCOUNT ON
	...
	values('ClientProperties', 'AUTH', @action, CASE WHEN LEN(HOST_NAME()) < 1 THEN ' ' ELSE HOST_NAME() END,
