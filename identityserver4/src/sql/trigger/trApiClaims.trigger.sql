DROP TRIGGER IF EXISTS AUTH.trApiClaims
GO

---
-- Trigger for ApiClaims that will handle logging of both update and delete
-- NOTE: inserted data is current value in row if not altered
---
CREATE TRIGGER trApiClaims
	ON [AUTH].[ApiClaims]
	FOR UPDATE, DELETE
	AS
		BEGIN
	...
	values('ApiClaims', 'AUTH', @action, CASE WHEN LEN(HOST_NAME()) < 1 THEN ' ' ELSE HOST_NAME() END,
