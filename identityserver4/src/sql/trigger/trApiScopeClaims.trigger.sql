DROP TRIGGER IF EXISTS AUTH.trApiScopeClaims
GO

---
-- Trigger for ApiScopeClaims that will handle logging of both update and delete
-- NOTE: inserted data is current value in row if not altered
---
CREATE TRIGGER trApiScopeClaims
	ON [AUTH].[ApiScopeClaims]
	FOR UPDATE, DELETE
	AS
		BEGIN
	SET NOCOUNT ON
	...
	values('ApiScopeClaims', 'AUTH', @action, CASE WHEN LEN(HOST_NAME()) < 1 THEN ' ' ELSE HOST_NAME() END,
