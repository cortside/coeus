DROP TRIGGER IF EXISTS AUTH.trApiResourceProperties
GO

---
-- Trigger for ApiResourceProperties that will handle logging of both update and delete
-- NOTE: inserted data is current value in row if not altered
---
CREATE TRIGGER trApiResourceProperties
	ON [AUTH].[ApiResourceProperties]
	FOR UPDATE, DELETE
	AS
		BEGIN
	SET NOCOUNT ON
	...
	values('ApiResourceProperties', 'AUTH', @action, CASE WHEN LEN(HOST_NAME()) < 1 THEN ' ' ELSE HOST_NAME() END,
