...
CREATE TRIGGER trUser
	...
	-- [SecurityToken]
	IF UPDATE([SecurityToken]) OR @action in ('INSERT', 'DELETE')      
		BEGIN       
			INSERT INTO audit.AuditLog (AuditLogTransactionId, PrimaryKey, ColumnName, OldValue, NewValue, Key1)
			SELECT
				@AuditLogTransactionId,
				convert(nvarchar(1500), IsNull('[[UserId]]='+CONVERT(nvarchar(4000), IsNull(OLD.[UserId], NEW.[UserId]), 0), '[[UserId]] Is Null')),
				'[SecurityToken]',
				CONVERT(nvarchar(4000), OLD.[SecurityToken], 126),
				CONVERT(nvarchar(4000), NEW.[SecurityToken], 126),
				convert(nvarchar(4000), COALESCE(OLD.[UserId], NEW.[UserId], null))
			FROM deleted OLD 
			LEFT JOIN inserted NEW On (NEW.[UserId] = OLD.[UserId] or (NEW.[UserId] Is Null and OLD.[UserId] Is Null))
			WHERE ((NEW.[SecurityToken] <> OLD.[SecurityToken]) 
					Or (NEW.[SecurityToken] Is Null And OLD.[SecurityToken] Is Not Null)
					Or (NEW.[SecurityToken] Is Not Null And OLD.[SecurityToken] Is Null))
			set @inserted = @inserted + @@ROWCOUNT
		END

	-- [TokenExpiration]
	IF UPDATE([TokenExpiration]) OR @action in ('INSERT', 'DELETE')      
		BEGIN       
			INSERT INTO audit.AuditLog (AuditLogTransactionId, PrimaryKey, ColumnName, OldValue, NewValue, Key1)
			SELECT
				@AuditLogTransactionId,
				convert(nvarchar(1500), IsNull('[[UserId]]='+CONVERT(nvarchar(4000), IsNull(OLD.[UserId], NEW.[UserId]), 0), '[[UserId]] Is Null')),
				'[TokenExpiration]',
				CONVERT(nvarchar(4000), OLD.[TokenExpiration], 126),
				CONVERT(nvarchar(4000), NEW.[TokenExpiration], 126),
				convert(nvarchar(4000), COALESCE(OLD.[UserId], NEW.[UserId], null))
			FROM deleted OLD 
			LEFT JOIN inserted NEW On (NEW.[UserId] = OLD.[UserId] or (NEW.[UserId] Is Null and OLD.[UserId] Is Null))
			WHERE ((NEW.[TokenExpiration] <> OLD.[TokenExpiration]) 
					Or (NEW.[TokenExpiration] Is Null And OLD.[TokenExpiration] Is Not Null)
					Or (NEW.[TokenExpiration] Is Not Null And OLD.[TokenExpiration] Is Null))
			set @inserted = @inserted + @@ROWCOUNT
		END
