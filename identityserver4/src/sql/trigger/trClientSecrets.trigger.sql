DROP TRIGGER IF EXISTS auth.trClientSecrets
GO

---
-- Trigger for ClientSecrets that will handle logging of both update and delete
-- NOTE: inserted data is current value in row if not altered
---
CREATE TRIGGER trClientSecrets
	ON [auth].[ClientSecrets]
	FOR UPDATE, DELETE
	AS
		BEGIN
	SET NOCOUNT ON
	...
	values('ClientSecrets', 'auth', @action, CASE WHEN LEN(HOST_NAME()) < 1 THEN ' ' ELSE HOST_NAME() END,
