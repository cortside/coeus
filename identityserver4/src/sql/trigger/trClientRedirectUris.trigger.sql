DROP TRIGGER IF EXISTS auth.trClientRedirectUris
GO

---
-- Trigger for ClientRedirectUris that will handle logging of both update and delete
-- NOTE: inserted data is current value in row if not altered
---
CREATE TRIGGER trClientRedirectUris
	ON [auth].[ClientRedirectUris]
	FOR UPDATE, DELETE
	AS
		BEGIN
	SET NOCOUNT ON
	...
	values('ClientRedirectUris', 'auth', @action, CASE WHEN LEN(HOST_NAME()) < 1 THEN ' ' ELSE HOST_NAME() END,
	...
	-- [RedirectUri]
	IF UPDATE([RedirectUri]) OR @action in ('INSERT', 'DELETE')      
		BEGIN       
			INSERT INTO audit.AuditLog (AuditLogTransactionId, PrimaryKey, ColumnName, OldValue, NewValue, Key1)
			SELECT
				@AuditLogTransactionId,
				convert(nvarchar(1500), IsNull('[[Id]]='+CONVERT(nvarchar(4000), IsNull(OLD.[Id], NEW.[Id]), 0), '[[Id]] Is Null')),
				'[RedirectUri]',
				CONVERT(nvarchar(4000), OLD.[RedirectUri], 126),
				CONVERT(nvarchar(4000), NEW.[RedirectUri], 126),
				convert(nvarchar(4000), COALESCE(OLD.[Id], NEW.[Id], null))
			FROM deleted OLD 
			LEFT JOIN inserted NEW On (NEW.[Id] = OLD.[Id] or (NEW.[Id] Is Null and OLD.[Id] Is Null))
			WHERE ((NEW.[RedirectUri] <> OLD.[RedirectUri]) 
					Or (NEW.[RedirectUri] Is Null And OLD.[RedirectUri] Is Not Null)
					Or (NEW.[RedirectUri] Is Not Null And OLD.[RedirectUri] Is Null))
			set @inserted = @inserted + @@ROWCOUNT
		END
