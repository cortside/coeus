DROP TRIGGER IF EXISTS AUTH.trApiResources
GO

---
-- Trigger for ApiResources that will handle logging of both update and delete
-- NOTE: inserted data is current value in row if not altered
---
CREATE TRIGGER trApiResources
	ON [AUTH].[ApiResources]
	FOR UPDATE, DELETE
	AS
		BEGIN
	SET NOCOUNT ON
	...
	values('ApiResources', 'AUTH', @action, CASE WHEN LEN(HOST_NAME()) < 1 THEN ' ' ELSE HOST_NAME() END,
	...
	IF UPDATE([AllowedAccessTokenSigningAlgorithms]) OR @action in ('INSERT', 'DELETE')      
		BEGIN       
			INSERT INTO audit.AuditLog (AuditLogTransactionId, PrimaryKey, ColumnName, OldValue, NewValue, Key1)
			SELECT
				@AuditLogTransactionId,
				convert(nvarchar(1500), IsNull('[[Id]]='+CONVERT(nvarchar(4000), IsNull(OLD.[Id], NEW.[Id]), 0), '[[Id]] Is Null')),
				'[AllowedAccessTokenSigningAlgorithms]',
				CONVERT(nvarchar(4000), OLD.[AllowedAccessTokenSigningAlgorithms], 126),
				CONVERT(nvarchar(4000), NEW.[AllowedAccessTokenSigningAlgorithms], 126),
				convert(nvarchar(4000), COALESCE(OLD.[Id], NEW.[Id], null))
			FROM deleted OLD 
			LEFT JOIN inserted NEW On (NEW.[Id] = OLD.[Id] or (NEW.[Id] Is Null and OLD.[Id] Is Null))
			WHERE ((NEW.[AllowedAccessTokenSigningAlgorithms] <> OLD.[AllowedAccessTokenSigningAlgorithms]) 
					Or (NEW.[AllowedAccessTokenSigningAlgorithms] Is Null And OLD.[AllowedAccessTokenSigningAlgorithms] Is Not Null)
					Or (NEW.[AllowedAccessTokenSigningAlgorithms] Is Not Null And OLD.[AllowedAccessTokenSigningAlgorithms] Is Null))
			set @inserted = @inserted + @@ROWCOUNT
		END
