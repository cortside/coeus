...
CREATE TRIGGER trClients
	...
	-- [DeviceCodeLifetime]
	IF UPDATE([DeviceCodeLifetime]) OR @action in ('INSERT', 'DELETE')      
		BEGIN       
			INSERT INTO audit.AuditLog (AuditLogTransactionId, PrimaryKey, ColumnName, OldValue, NewValue, Key1)
			SELECT
				@AuditLogTransactionId,
				convert(nvarchar(1500), IsNull('[[Id]]='+CONVERT(nvarchar(4000), IsNull(OLD.[Id], NEW.[Id]), 0), '[[Id]] Is Null')),
				'[DeviceCodeLifetime]',
				CONVERT(nvarchar(4000), OLD.[DeviceCodeLifetime], 126),
				CONVERT(nvarchar(4000), NEW.[DeviceCodeLifetime], 126),
				convert(nvarchar(4000), COALESCE(OLD.[Id], NEW.[Id], null))
			FROM deleted OLD 
			LEFT JOIN inserted NEW On (NEW.[Id] = OLD.[Id] or (NEW.[Id] Is Null and OLD.[Id] Is Null))
			WHERE ((NEW.[DeviceCodeLifetime] <> OLD.[DeviceCodeLifetime]) 
					Or (NEW.[DeviceCodeLifetime] Is Null And OLD.[DeviceCodeLifetime] Is Not Null)
					Or (NEW.[DeviceCodeLifetime] Is Not Null And OLD.[DeviceCodeLifetime] Is Null))
			set @inserted = @inserted + @@ROWCOUNT
		END

	-- [LastAccessed]
	IF UPDATE([LastAccessed]) OR @action in ('INSERT', 'DELETE')      
		BEGIN       
			INSERT INTO audit.AuditLog (AuditLogTransactionId, PrimaryKey, ColumnName, OldValue, NewValue, Key1)
			SELECT
				@AuditLogTransactionId,
				convert(nvarchar(1500), IsNull('[[Id]]='+CONVERT(nvarchar(4000), IsNull(OLD.[Id], NEW.[Id]), 0), '[[Id]] Is Null')),
				'[LastAccessed]',
				CONVERT(nvarchar(4000), OLD.[LastAccessed], 126),
				CONVERT(nvarchar(4000), NEW.[LastAccessed], 126),
				convert(nvarchar(4000), COALESCE(OLD.[Id], NEW.[Id], null))
			FROM deleted OLD 
			LEFT JOIN inserted NEW On (NEW.[Id] = OLD.[Id] or (NEW.[Id] Is Null and OLD.[Id] Is Null))
			WHERE ((NEW.[LastAccessed] <> OLD.[LastAccessed]) 
					Or (NEW.[LastAccessed] Is Null And OLD.[LastAccessed] Is Not Null)
					Or (NEW.[LastAccessed] Is Not Null And OLD.[LastAccessed] Is Null))
			set @inserted = @inserted + @@ROWCOUNT
		END
	...
	-- [UserCodeType]
	IF UPDATE([UserCodeType]) OR @action in ('INSERT', 'DELETE')      
		BEGIN       
			INSERT INTO audit.AuditLog (AuditLogTransactionId, PrimaryKey, ColumnName, OldValue, NewValue, Key1)
			SELECT
				@AuditLogTransactionId,
				convert(nvarchar(1500), IsNull('[[Id]]='+CONVERT(nvarchar(4000), IsNull(OLD.[Id], NEW.[Id]), 0), '[[Id]] Is Null')),
				'[UserCodeType]',
				CONVERT(nvarchar(4000), OLD.[UserCodeType], 126),
				CONVERT(nvarchar(4000), NEW.[UserCodeType], 126),
				convert(nvarchar(4000), COALESCE(OLD.[Id], NEW.[Id], null))
			FROM deleted OLD 
			LEFT JOIN inserted NEW On (NEW.[Id] = OLD.[Id] or (NEW.[Id] Is Null and OLD.[Id] Is Null))
			WHERE ((NEW.[UserCodeType] <> OLD.[UserCodeType]) 
					Or (NEW.[UserCodeType] Is Null And OLD.[UserCodeType] Is Not Null)
					Or (NEW.[UserCodeType] Is Not Null And OLD.[UserCodeType] Is Null))
			set @inserted = @inserted + @@ROWCOUNT
		END

	-- [UserSsoLifetime]
	IF UPDATE([UserSsoLifetime]) OR @action in ('INSERT', 'DELETE')      
		BEGIN       
			INSERT INTO audit.AuditLog (AuditLogTransactionId, PrimaryKey, ColumnName, OldValue, NewValue, Key1)
			SELECT
				@AuditLogTransactionId,
				convert(nvarchar(1500), IsNull('[[Id]]='+CONVERT(nvarchar(4000), IsNull(OLD.[Id], NEW.[Id]), 0), '[[Id]] Is Null')),
				'[UserSsoLifetime]',
				CONVERT(nvarchar(4000), OLD.[UserSsoLifetime], 126),
				CONVERT(nvarchar(4000), NEW.[UserSsoLifetime], 126),
				convert(nvarchar(4000), COALESCE(OLD.[Id], NEW.[Id], null))
			FROM deleted OLD 
			LEFT JOIN inserted NEW On (NEW.[Id] = OLD.[Id] or (NEW.[Id] Is Null and OLD.[Id] Is Null))
			WHERE ((NEW.[UserSsoLifetime] <> OLD.[UserSsoLifetime]) 
					Or (NEW.[UserSsoLifetime] Is Null And OLD.[UserSsoLifetime] Is Not Null)
					Or (NEW.[UserSsoLifetime] Is Not Null And OLD.[UserSsoLifetime] Is Null))
			set @inserted = @inserted + @@ROWCOUNT
		END
