DROP TRIGGER IF EXISTS AUTH.trIdentityProperties
GO

---
-- Trigger for IdentityProperties that will handle logging of both update and delete
-- NOTE: inserted data is current value in row if not altered
---
CREATE TRIGGER trIdentityProperties
	ON [AUTH].[IdentityProperties]
	FOR UPDATE, DELETE
	AS
		BEGIN
	SET NOCOUNT ON
	...
	values('IdentityProperties', 'AUTH', @action, CASE WHEN LEN(HOST_NAME()) < 1 THEN ' ' ELSE HOST_NAME() END,
