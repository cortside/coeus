DROP TRIGGER IF EXISTS auth.trClientCorsOrigins
GO

---
-- Trigger for ClientCorsOrigins that will handle logging of both update and delete
-- NOTE: inserted data is current value in row if not altered
---
CREATE TRIGGER trClientCorsOrigins
	ON [auth].[ClientCorsOrigins]
	FOR UPDATE, DELETE
	AS
		BEGIN
	SET NOCOUNT ON
	...
	values('ClientCorsOrigins', 'auth', @action, CASE WHEN LEN(HOST_NAME()) < 1 THEN ' ' ELSE HOST_NAME() END,
	...
	-- [Origin]
	IF UPDATE([Origin]) OR @action in ('INSERT', 'DELETE')      
		BEGIN       
			INSERT INTO audit.AuditLog (AuditLogTransactionId, PrimaryKey, ColumnName, OldValue, NewValue, Key1)
			SELECT
				@AuditLogTransactionId,
				convert(nvarchar(1500), IsNull('[[Id]]='+CONVERT(nvarchar(4000), IsNull(OLD.[Id], NEW.[Id]), 0), '[[Id]] Is Null')),
				'[Origin]',
				CONVERT(nvarchar(4000), OLD.[Origin], 126),
				CONVERT(nvarchar(4000), NEW.[Origin], 126),
				convert(nvarchar(4000), COALESCE(OLD.[Id], NEW.[Id], null))
			FROM deleted OLD 
			LEFT JOIN inserted NEW On (NEW.[Id] = OLD.[Id] or (NEW.[Id] Is Null and OLD.[Id] Is Null))
			WHERE ((NEW.[Origin] <> OLD.[Origin]) 
					Or (NEW.[Origin] Is Null And OLD.[Origin] Is Not Null)
					Or (NEW.[Origin] Is Not Null And OLD.[Origin] Is Null))
			set @inserted = @inserted + @@ROWCOUNT
		END
