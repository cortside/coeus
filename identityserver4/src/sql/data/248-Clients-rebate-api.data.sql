Declare @clientName varchar(200) ='Rebate Api'
Declare @clientId varchar(200) ='rebate-api'
Declare @id int
...
if (not exists(select * from auth.ClientScopes where clientid = @id and scope = 'groups'))
  ...
  END
   ...
   if (not exists(select * from auth.ClientScopes where clientid = @id and scope = 'partnerportal.api'))
  BEGIN
    INSERT [AUTH].[ClientScopes] ([ClientId], [Scope]) VALUES (@id, N'partnerportal.api')
  END
...
if (not exists(select * from auth.ClientScopes where clientid = @id and scope = 'policyserver.runtime'))
  ...
  if (not exists(select * from auth.ClientClaims where clientid = @id and Type='sub' and Value = 'f20b4e2a-0b82-40ea-bd27-52309e089fe1'))
  BEGIN
  	insert into auth.ClientClaims values(@id, 'sub', 'f20b4e2a-0b82-40ea-bd27-52309e089fe1')
  END
