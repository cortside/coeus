...
declare @id int
set @clientId='ppl-orgservice-client'
...
if not exists(select * from auth.ClientClaims where clientId=@id)
  begin
	...
	insert into auth.ClientClaims values(@id, 'role', 'ppl-orgservice-client')
	insert into auth.ClientClaims values(@id, 'sub', '477f24ea-c680-4b45-943c-43e0b893be12')
  end
