...
if not exists(select * from auth.clientscopes where clientId=@id)
  begin
	...
	INSERT [AUTH].[ClientScopes] ([ClientId], [Scope]) VALUES (@id, N'lenderaudit.api')
  end

if not exists(select * from auth.ClientClaims where clientId=@id)
  begin
	-- MUST be unique across all clients and users (subjects)
	insert into auth.ClientClaims values(@id, 'role', 'lenderaudit.api')
	insert into auth.ClientClaims values(@id, 'sub', 'e6f7e57f-4109-487b-b40b-d9d680082e1e')
  end
