...
if (not exists(select * from auth.[ApiResources] where name=@name))
  BEGIN
	...
	-- Zgmp0sk7CvCK98uY
	INSERT [AUTH].[ApiResourceSecrets] ([ApiResourceId], [Description], [Expiration], [Type], [Value]) VALUES (@id, @name, CAST(N'2099-12-31T00:00:00.0000000' AS DateTime2), N'Resource', N'7UZuvaGmyelfq116ybybORCju9kZp9vuDLDLZ4YyYNI=')
  END
  ...
  BEGIN
	...
	if (not exists(SELECT * FROM [AUTH].[ApiResourceSecrets] where ApiResourceId=@id AND Type = 'SharedSecret'))
		BEGIN
			...
			INSERT [AUTH].[ApiResourceSecrets] ([ApiResourceId], [Description], [Expiration], [Type], [Value]) VALUES (@id, @name, CAST(N'2099-12-31T00:00:00.0000000' AS DateTime2), N'SharedSecret', N'hqe1WY3/qBpZcIjv4ju5k97mdOUMRscgiDItujarKAc=')
		END
  END
