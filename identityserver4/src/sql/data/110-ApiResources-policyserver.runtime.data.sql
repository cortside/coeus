...
if (not exists(select * from auth.[ApiResources] where name=@name))
  ...
  -- "[Value]" contains a base64 encoding of a SHA256 hash of a random string.
	...
	-- policypass
	INSERT [AUTH].[ApiSecrets] ([ApiResourceId], [Description], [Expiration], [Type], [Value]) VALUES (@id, N'policyserver.runtime', CAST(N'2099-12-31T00:00:00.0000000' AS DateTime2), N'Resource', N'V3J3sVs+undm/4pZyp16++QO+TQSHmxjx/jfshX2qtM=')
