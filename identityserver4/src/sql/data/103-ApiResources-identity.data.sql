...
--select @hb as [BinaryHash], @h64 as [64Hash];
	...
	INSERT [AUTH].[ApiSecrets] ([ApiResourceId], [Description], [Expiration], [Type], [Value]) VALUES (@id, @name, CAST(N'2099-12-31T00:00:00.0000000' AS DateTime2), N'Resource', N'lam6cNU2H3t93VlA1M11H0fZFAOUDZXqpsjpCgSlPOc=')
  END

select top 1 @id = Id from [AUTH].[ApiResources] where [Name] = @name

if not exists (select top 1 1 from Auth.ApiSecrets where ApiResourceId = @id and [Description] = 'identity introspection') begin
	insert into Auth.ApiSecrets (ApiResourceId, [Description], Expiration, [Type], [Value]) values (@id, 'identity introspection', '2099-12-31', 'SharedSecret', 'K7gNU3sdo+OL0wNhqoVWhr3g6s1xYv72ol/pe/Unols=')
end