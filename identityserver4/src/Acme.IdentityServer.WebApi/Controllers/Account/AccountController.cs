...
using Acme.IdentityServer.WebApi;
...
namespace Acme.IdentityServer.Controllers.Account {
    ...
    public class AccountController : Controller {
                ...
                if (user != null) {
                    if (user.TwoFactorVerified) {
                        ...
                        if (!bool.Parse(config["requireTOTPForLocal"])) {
                            await SignInUserAsync(AccountOptions.AllowRememberLogin, model.RememberLogin,
                                AccountOptions.RememberMeLoginDuration, user);
                            var returnUrl = model.ReturnUrl;
                            logger.LogInformation($"Return URL: {returnUrl}");
                            if (idsService.IsValidReturnUrl(returnUrl) || Url.IsLocalUrl(returnUrl)) {
                                return Redirect(returnUrl);
        ...
        private async Task SignInUserAsync(bool allowRememberLogin, bool rememberLogin, TimeSpan rememberMeLoginDuration, User user) {
            AuthenticationProperties props = null;
